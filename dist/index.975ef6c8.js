...
var _apiServiceJs = require("./js/components/apiService.js");
...
var _svg = require("./js/components/svg");
...
console.log(list);
input.addEventListener("input", (0, _lodashDebounceDefault.default)(onSearch, 1000));
console.log(input);
function onSearch() {
    list.innerHTML = "";
    const searchQuery = input.query.value;
    console.log(searchQuery);
    const fetchQuery = (0, _apiServiceJsDefault.default)(searchQuery);
    fetchQuery.then((data)=>{
        if (data.hits.length === 0) {
            console.log("ну наконец-то заработало)))");
            (0, _notiflixDefault.default).Notify.success("К сожалению, нет изображений, соответствующих вашему поисковому запросу. Пожалуйста, введите что то другое");
        }
    });
    fetchQuery.then((data)=>data.hits.map((e)=>list.insertAdjacentHTML("beforeend", `<div class="photo-card">
          <div class="photo-wrapper"> <img src="${e.webformatURL}" alt="${e.tags}" loading="lazy" width="300px"/></div>
            <div class="info">
              <p class="info-item">${(0, _svgDefault.default).likes}${e.likes}</p>
              <p class="info-item">${(0, _svgDefault.default).views}${e.views}</p>
              <p class="info-item">${(0, _svgDefault.default).coments}${e.comments}</p>
              <p class="info-item">${(0, _svgDefault.default).down} ${e.downloads}</p>
            </div>
        </div>`)));
// .catch(
...
},{"./sass/index.scss":"9OQSl","./js/components/apiService.js":"9ZEWI","lodash.debounce":"3JP5n","./js/components/svg":"bDdpk","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","notiflix":"5z0Oc"}],"9OQSl":[function() {},{}],"9ZEWI":[function(require,module,exports) {
...
function fetchCountries(searchQuery) {
    ...
    let PAGE = "1";
    return fetch(`${BASE_URL}?key=${KEY}&q=${searchQuery}}&image_type=photo&per_page=12&page=${PAGE}`).then((response)=>response.json());
}
...
},{}]},["jC2qd","8lqZg"], "8lqZg", "parcelRequirec169")
